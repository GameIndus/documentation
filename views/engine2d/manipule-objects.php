...
<br>
<p>
	Avant de pouvoir manipuler et modifier les paramètres d'un objet d'une scène, il faut tout d'abord le récupérer et le stocker dans une variable <small>(non obligatoire)</small>.
...
<blockquote>
	<p>
		<i class="fa fa-info-circle"></i> Le nom d'une scène correspond à son nom de fichier. Le nom d'un objet de jeu correspond à son nom donné dans l'éditeur de scène, dans la boîte de paramètres.
	</p>
</blockquote>
<br>
...
<br>
<p>
	Il existe de nombreuses fonctions communes à tous les objets de jeu. On peut notamment citer les méthodes de position, de taille, etc.. Voici une liste
	détaillée de ces fonctions :

	<ul style="list-style-type:square">
		<li><b>getID()</b> : récupère l'identifiant de l'objet</li>
		<li><b>getPosition()</b> : récupère la position de l'objet. On peut ensuite manipuler cette position grâce à de nombreuses fonctions comme <i>getY()</i>, <i>setX()</i>, <i>addX()</i>, <i>addY()</i>, <i>substract()</i>...</li>
		...
		<li><b>addBehavior(b)</b> : Attacher un comportement à l'objet {b: classe de comportement}. <u>Voir la section appropriée pour plus d'infos</u></li>
		<li><b>setLayer(layer)</b> : Change le calque d'apparition de l'objet dans la scène (de 0 à 9). Plus l'objet sera sur un calque élevé, plus il sera mis en avant.</li>
		<li><b>setOpacity(o)</b> : Change l'opacité {o} de l'objet (de 0 à 1). Mettre l'opacité à 0 revient à cacher l'objet</li>
...
<p>
	...
	<ul style="list-style-type:square">
		<li>
			...
			on peut donc faire déplacer l'objet avec une direction, une force et un sens. On peut manipuler les vecteurs grâce à la classe Vector2. Voici un exemple :
...
o.setVelocity(v); // Enfin, on applique notre velocité à l'objet stocké dans la variable "o"</code></pre>
		...
		<li>
			<b>Moteur de rendu</b> : un moteur de rendu va permettre à l'objet d'être affiché à l'écran. Un objet sans moteur de rendu <u>apparaîtera invisible</u>. 
			Il ne faut donc pas faire abstraction de cela. Il en existe 3 à l'heure actuelle : ImageRenderer, SpriteRenderer et GeometricRenderer. Si cela vous paraît trop difficile,
...
<p>
	...
	<ul style="list-style-type:square">
		<li>
			<b><span class="err"><i class="fa fa-warning"></i></span> defineAnimation(name, speed, startPos, frames, flipped)</b> : <span class="err">Niveau avancé requis</span>.
			Créer une animation spécifique avec un nom {name}, une vitesse {speed}, une position de départ {startPos}, les images à suivre (indexs de l'image) {frames}. Il est aussi
...
<br>
<p>
	Parfois on souhaite effectuer des opérations plus spécifiques sur un objet. Voici quelques mises en scènes dans lesquelles vous pourrez tomber :

	<ul style="list-style-type:square">
		<li>
			<b>Je veux récupérer un objet de ma scène à une certaine position (x, y).</b>
			<p>
				Vous pouvez le faire grâce à la méthode <i>getObjectAt(x, y, layer, exclude)</i> de la classe scène. Vous devez fournir la position à tester {x} et {y},
...
// Cette fonction va chercher un objet aux cordonnées {500, 500} au calque 2, sans exclure d'objet.</code></pre>
		...
		<li>
			...
			<p>
				Il est tout à fait possible de le faire, en instanciant la classe <i>GameObject</i> avec un tableau contenant la taille de l'objet en paramètre. Vous devrez néanmoins l'ajouter à votre scène grâce à la fonction
...
Game.getCurrentScene().add(objet); // Je l'ajoute à ma scène courrante</code></pre>
		...
		<li>
			...
			<p>
				Cette opération est réalisable grâce à la fonction <i>distanceTo</i>. Plus concrètement, elle permet de récupérer la distance entre deux positions.
				<br><br>
...
console.log(dist); // J'affiche la distance dans la console de développement.</code></pre>
		...
		<li>
			...
			<p>
				Cette opération est réalisable grâce à la fonction <i>lerpTo</i>. "lerp" est une fonction mathématique permettant de créer une interpolation entre deux valeurs. Si cette définition ne vous dis rien, ce n'est pas grave : cela permet de fluidifier les déplacements d'une position à une autre.
				<br><br>
				<pre><code class="javascript">var scene = Game.getCurrentScene(); // Je récupère la scène courrante
var objet1 = scene.getGameObject("Objet 1"); // Je récupère mon objet "Objet 1", créé dans l'éditeur de scène
var objet2 = scene.getGameObject("Objet 2"); // Je récupère mon objet "Objet 2"
